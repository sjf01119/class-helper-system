  ...
  <div class="log-page">
    <el-card class="search-card" shadow="never">
      <el-form :model="queryParams" inline>
        ...
        <el-form-item label="操作类型">
          <el-input v-model="queryParams.operation" placeholder="请输入操作类型" clearable style="width: 160px" />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="logList" v-loading="loading" border stripe>
        ...
        <el-table-column prop="operation" label="操作类型" width="150" show-overflow-tooltip />
        ...
        <el-table-column prop="params" label="请求参数" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <el-button link type="primary" @click="showParams(row)">查看参数</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="ip" label="操作IP" width="140" />
        <el-table-column prop="errorMsg" label="错误信息" min-width="150" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.errorMsg" class="fail-reason">{{ row.errorMsg }}</span>
    ...
    <!-- 参数详情弹窗 -->
    <el-dialog title="请求参数" v-model="paramsDialogVisible" width="600px">
...
import { getOperationLogPage, type OperationLog } from '@/api/log'
...
const showParams = (row: OperationLog) => {
  try {
    currentParams.value = row.params ? JSON.stringify(JSON.parse(row.params), null, 2) : '无参数'
  } catch {
    currentParams.value = row.params || '无参数'
  }
