  ...
  <div class="announcement-manage">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="标题" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="title-cell">
              ...
              <el-tag :type="row.publishScope === 0 ? 'primary' : 'success'" size="small">
        ...
        <el-table-column label="发布范围" width="130">
          <template #default="{ row }">
            {{ row.publishScope === 1 ? '班级公告' : '系统公告' }}
          </template>
        </el-table-column>
    ...
    <!-- 查看弹窗 -->
    <el-dialog title="公告详情" v-model="viewDialogVisible" width="650px">
...
const loadClassList = async () => {
  try {
    ...
    classList.value = res || []
    if (!formData.classId && classList.value.length > 0) {
      formData.classId = classList.value[0].id
    }
  } catch (error) {
...
const handleAdd = () => {
  ...
  Object.assign(formData, {
    ...
    type: 2,
    publishScope: 1,
    classId: classList.value.length > 0 ? classList.value[0].id : undefined,
    isTop: 0,
    status: 1
  })
...
const handleEdit = (row: Announcement) => {
  ...
  Object.assign(formData, {
    ...
    content: row.content,
    type: row.type,
    publishScope: row.publishScope ?? 1,
    classId: row.classId,
